...
performance in several ojectives. Using 200m resolution.
Was having problems with integers being too large for the array but fixing
by making them all doubles

Now I'm using 200 metre resolution need to double the number of dwellings.
This is achieved by multiplying by 4
...
def Plan(Development_Plan):
    ...
    import Undeveloped_Lookup_200 as Lookup
...
def Calc_fheat(Development_Plan):
    ...
    # Assign development Sites to Entire London and multiply by 2
    # As its at 200 m spatial resolution
    Lndn_Dev_Plan = np.multiply(Plan(Development_Plan),4)
...
def Calc_fflood(Development_Plan):
    ...
    # Assign development Sites to Entire London and multiply by 4
    # As its at 200 m spatial resolution
    Lndn_Dev_Plan = np.multiply(Plan(Development_Plan),4)
...
def Calc_fregeneration(Development_Plan):
    ...
    Per_Regenerate  = (float(Regen_Correlates)/float(Total_Sites)) * 100  
    return Per_Regenerate
