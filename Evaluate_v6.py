...
def Calc_fheat(London_Dwell_Plan):
...
def Calc_fflood(London_Dwell_Plan):
    ...
    FloodRisk = np.multiply(London_Dwell_Plan, Floodzone_Reduced)
...
def Calc_fbrownfield(London_DevPlan, London_DwellPlan):
    ...
    # Objectie function is calculated based on number of proposed dwellings 
    # which can be built on brownfield compared to total number of dwellings
    ...
    Brownfield_Avai = np.double(rasterIO.readrasterband(rasterIO.opengdalraster(data_folder+'Brwnfld_per.tif'),1))


    # Calculates number of Dwellings on each dev site can be built based on the
    # brownfield space available
    Brownfield_Dwell_Plan =  np.multiply(0.25,np.multiply(Brownfield_Avai, London_DevPlan))
    
    #Calculate the sum of all dwellings on brownfield
    a = np.array(Brownfield_Dwell_Plan)
    b = a.ravel()
    Brownfield_Dwells_Total = sum(b)
    
    e = np.array(London_DwellPlan)
    f = e.ravel()
    Dwells_Total = sum(f)
    
    #print "b total", Brownfield_Dwells_Total
    #print "total", Dwells_Total
    ...
    Per_Not_Brownfield  = (1-(float(Brownfield_Dwells_Total)/float(Dwells_Total)))* 100
    
    
    #return Per_Brownfield
    return Per_Not_Brownfield
    
def Calc_fsprawl(London_Dwell_Plan):
    ...
    Sites_within_Urban_Extent = np.count_nonzero(np.multiply(Urban_Extent, London_Dwell_Plan))
