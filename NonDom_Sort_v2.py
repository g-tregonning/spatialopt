...
def Sort(solutions, ObjFunc):
    ...
    Sorted_Solution_list = sorted(Solution_list, key=lambda tup: tup.fitness.values[ObjFunc[0]])
    #Solution_list.sort(key=lambda x: x.fitness.values[ObjFunc[0]], reverse = False) #currently sorted by smallest 1st obj    
    #print solutions
    NonDom_list.append(Sorted_Solution_list[0])
    #NonDom_list.append(Solution_list[0]) #
    Sorted_Solution_list.pop(0) 
    for Sol in Sorted_Solution_list: #Check each solution in the solution list
        ...
        for NonDom_Sol in NonDom_list:
            ...
            #print NonDom_Sol
            #ObjFunc
            Dominated, Dominates= Domination_Check(Sol.fitness.values,NonDom_Sol.fitness.values,ObjFunc)
            if Dominated == True:
